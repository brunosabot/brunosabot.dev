 ...
 */
const gist = () => async (tree, file) => {
  let _i, _j;

  await visit(tree, "paragraph", (node, i) => {
    _i = i;
    // console.log(i, rest)
    return Promise.all(
      node.children.map(async (child, j) => {
        if (child.type === "inlineCode" && child.value.startsWith("gist:")) {
          _j = j;
          const query = getQuery(child.value.substring(5));
          const url = buildUrl(child.value.substring(5), {}, query.file);
          ...
          tree.children[i].children[j].type = "html";
          tree.children[i].children[j].value = html.trim();
        }
      })
    );
  });
